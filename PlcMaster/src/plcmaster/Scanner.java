...
public class Scanner {
	...
	static {
		...
		for (int i = 48; i <= 57; ++i) start.set(i, 5);
		for (int i = 65; i <= 90; ++i) start.set(i, 6);
		for (int i = 95; i <= 95; ++i) start.set(i, 6);
		for (int i = 97; i <= 122; ++i) start.set(i, 6);
		start.set(34, 1); 
		start.set(59, 9); 
		start.set(58, 10); 
		start.set(40, 12); 
		...
		literals.put("If", new Integer(6));
		literals.put("Then", new Integer(7));
		literals.put("Else", new Integer(8));
		literals.put("End", new Integer(9));
		literals.put("Signal", new Integer(10));
		literals.put("True", new Integer(12));
		literals.put("False", new Integer(13));
		literals.put("NotConnected", new Integer(14));
		literals.put("IsConnected", new Integer(15));
		literals.put("Not", new Integer(16));
		literals.put("And", new Integer(19));
		literals.put("Or", new Integer(20));
		literals.put("Xor", new Integer(21));
	...
	Token NextToken() {
		...
		loop: for (;;) {
			switch (state) {
				...
				case 1:
					if (ch <= '!' || ch >= '#' && ch <= 65535) {AddCh(); state = 3; break;}
				...
				case 6:
					...
					if (ch >= '0' && ch <= '9' || ch >= 'A' && ch <= 'Z' || ch == '_' || ch >= 'a' && ch <= 'z') {AddCh(); state = 6; break;}
					else if (ch == '.') {AddCh(); state = 7; break;}
					else {t.kind = 3; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
				case 7:
					recEnd = pos; recKind = 3;
					if (ch == '.' || ch >= '0' && ch <= '9' || ch >= 'A' && ch <= 'Z' || ch == '_' || ch >= 'a' && ch <= 'z') {AddCh(); state = 7; break;}
					else {t.kind = 3; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
				case 8:
					recEnd = pos; recKind = 1;
					if (ch == '"') {AddCh(); state = 2; break;}
